...
void scene5( Viewer& viewer )
{
    ...
    traing->addGlobalTransformKeyframe(
        getTranslationMatrix(-52, 0, -5)*getRotationMatrix(3.14/2, glm::vec3(0, -1, 0)),
        10
    );
    /*traing->addGlobalTransformKeyframe(
        getTranslationMatrix(-52, 0, -5)*getRotationMatrix(3.14/2, glm::vec3(0, -1, 0))*getRotationMatrix(3.14/2, glm::vec3(0, 0, -1)),
        12
    );
    traing->addGlobalTransformKeyframe(
        getTranslationMatrix(-52, 0, -5)*getRotationMatrix(3.14/2, glm::vec3(0, -1, 0))*getRotationMatrix(3.14/2, glm::vec3(0, 0, -1)),
    ...
    // Add a slalom effect to the car
    float startX = 50.0f;
    float endX = -40.0f;
    float y = 2.0f;
    float z = -13.0f;
    float scale = 0.4f;
    int totalFrames = 10;
    float stepX = (endX - startX) / totalFrames;
    float zigzagAmplitude = 0.7f;
    float rotationAngle = glm::radians(1.0f); // Adjust the rotation angle for the slalom effect

    for (int i = 0; i <= totalFrames; ++i) {
        float currentX = startX + i * stepX;
        float currentZ = z + ((i % 2 == 0) ? zigzagAmplitude : -zigzagAmplitude);
        glm::mat4 translation = getTranslationMatrix(currentX, y, currentZ);
        glm::mat4 scaleMatrix = getScaleMatrix(scale, scale, scale);
        glm::mat4 rotation = getRotationMatrix((i % 2 == 0) ? rotationAngle : -rotationAngle, glm::vec3(0, 1, 0));

        car->addGlobalTransformKeyframe(translation * rotation * scaleMatrix, i);
    }
    /*car->addGlobalTransformKeyframe(
        getTranslationMatrix(-45, 2, -5)*getScaleMatrix(0.4, 0.4, 0.4),
        11
    );
    car->addGlobalTransformKeyframe(
        getTranslationMatrix(-45, 2, -5)*getScaleMatrix(0.4, 0.4, 0.4),
