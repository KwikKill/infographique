...
TexturedLightedMeshRenderablePtr init_bridge(ShaderProgramPtr phong_shader) {
    // Add the bridge
    const std::string bridge_path = "../../good/obj/bridge.obj";
    std::string image_path = "./../good/Material_baseColor.png";
    ...
    TexturedLightedMeshRenderablePtr root_bridge;
    ...
    */
    root_bridge = std::make_shared<TexturedLightedMeshRenderable>(
        phong_shader, bridge_path, materials_bridge[0], image_path);
...
void test( Viewer& viewer )
{
    ...
    // Add the bridge
    TexturedLightedMeshRenderablePtr bridge = init_bridge(phong_shader);
    viewer.addRenderable(bridge);
