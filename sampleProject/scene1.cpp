...
LightedMeshRenderablePtr init_sign(ShaderProgramPtr phong_shader)
{
...
void scene1(Viewer &viewer)
{
    ...
    // Add the 9 3/4 sign
    LightedMeshRenderablePtr sign_root = init_sign(phong_shader);

    // resize and move the plateform
    sign_root->setGlobalTransform(
        getTranslationMatrix(6, 3, -8) * getScaleMatrix(1, 1, 1));
