...
LightedMeshRenderablePtr init_cartest( ShaderProgramPtr phong_shader) {
    // Add the car
    const std::string car_path = "../../good/obj/musclecar.obj";
    std::vector<std::vector<glm::vec3>> all_positions_car;
    std::vector<std::vector<glm::vec3>> all_normals_car;
    std::vector<std::vector<glm::vec2>> all_texcoords_car;
    std::vector<std::vector<unsigned int>> all_indices_car;
    std::vector<MaterialPtr> materials_car;

    read_obj_with_materials(car_path, texture_path, all_positions_car, all_normals_car, all_texcoords_car, materials_car);

    int n_object_car = materials_car.size();
    std::vector<glm::vec4> colors_car;

    LightedMeshRenderablePtr root_car;

    root_car = std::make_shared<LightedMeshRenderable>(
        phong_shader, all_positions_car[0], all_normals_car[0], colors_car, materials_car[0]);
    for (int i = 1 ; i < n_object_car ; ++i){
        LightedMeshRenderablePtr part = std::make_shared<LightedMeshRenderable>(
        phong_shader, all_positions_car[i], all_normals_car[i], colors_car, materials_car[i]);
        HierarchicalRenderable::addChild(root_car, part);
    }
...
void test( Viewer& viewer ) {
    ...
    std::string bunny_mesh_path = "../../good/pinguin.obj";
    std::string bunny_texture_path = "../../good/pinguin_color3.png";
    auto bunny = std::make_shared<TexturedMeshRenderable>(texShader, bunny_mesh_path, bunny_texture_path);
    viewer.addRenderable(bunny);*/

    LightedMeshRenderablePtr root = init_traing(phong_shader, 3, {});
    root->setGlobalTransform(getTranslationMatrix(0, 0, 0)*getRotationMatrix(3.14/2, glm::vec3(0, -1, 0)));
