    ...
    MeshRenderable(shaderProgram, indexed)
{
    if (indexed){
        ...
        // Set normal as colors
        m_colors.resize(m_positions.size(), glm::vec4(0));
        for (size_t i=0; i<m_colors.size(); ++i){
            glm::vec3 normal = glm::normalize(m_normals[i]);
            glm::vec3 color = (normal + glm::vec3(1.0f)) * 0.5f; // Remap from [-1, 1] to [0, 1]
            m_colors[i] = glm::vec4(color, 1.0f);
        }
    }
